...
Запуск:
    ...
    python tg_commenter.py

При первом запуске Pyrogram запросит номер телефона и код подтверждения.
Для управления ботом отправьте себе (в Saved Messages или с другого аккаунта)
секретный код из ADMIN_CODE — откроется панель управления.
"""
...
# ── Сессии админ-панели ───────────────────────────────────────────────────────
# user_id → True если пользователь прошёл аутентификацию через ADMIN_CODE
...
async def handle_admin_message(client, message) -> None:
    """
    Мониторит все входящие личные сообщения.
    Секретный код открывает панель управления.
    Дальнейшие команды выполняются в рамках активной сессии.
    """
    ...
    # ── /list ─────────────────────────────────────────────────────────────────
        ...
        if not state.channels:
            ...
            lines = "\n".join(
                f"{i + 1}. @{c}" for i, c in enumerate(state.channels)
            )
...
async def handle_post(client, message) -> None:
    """
    Срабатывает на каждый новый пост в каналах.
    Проверяет, нужно ли комментировать, ждёт случайную задержку,
    генерирует комментарий через Groq и отправляет как ответ на пост
    (попадает в группу обсуждения канала).
    """
    try:
        # ── Проверки ──────────────────────────────────────────────────────────
        ...
        # ── Генерация комментария (1 повтор при ошибке) ───────────────────────
